...
func NewStackedDA(g *ExprGraph, batchSize, size, inputs, outputs, layers int, hiddenSizes []int, corruptions []float64,
	trainingLog *log.Logger, dt tensor.Dtype) *StackedDA {
	...
	for i := 0; i < layers; i++ {
		...
		hiddenLayers[i] = NewFC(dt, WithGraph(g), WithConf(inputSize, outputSize, batchSize))
		...
		autoencoders[i] = NewDATiedWeights(hiddenLayers[i].w, hiddenLayers[i].b, corruptions[i], dt, WithGraph(g), WithConf(inputSize, outputSize, batchSize))
		autoencoders[i].input = input
	}

	final := NewSoftmaxLayer(dt, WithGraph(g), WithConf(hiddenSizes[len(hiddenSizes)-1], outputs, batchSize))
...
func (sda *StackedDA) Pretrain(x tensor.Tensor, epoch int) (err error) {
	...
	sda.trainingLog.Println(buf.String())
...
func (sda *StackedDA) Finetune(x tensor.Tensor, y []int, epoch int) (err error) {
	...
	sda.trainingLog.Printf("%d\t%v", epoch, avgF64s(cvs))
	return nil
}
