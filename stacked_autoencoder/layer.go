...
func NewFC(dt tensor.Dtype, opts ...LayerConsOpt) *FC {
	fc := new(FC)
	fc.af = Sigmoid
	...
	fc.Neuron = MakeNeuron(fc.Inputs, fc.Outputs, fc.BatchSize, fc.g, u, dt)
	return fc
}
...
func NewSoftmaxLayer(dt tensor.Dtype, opts ...LayerConsOpt) *SoftmaxLayer {
	sm := new(SoftmaxLayer)
	for _, opt := range opts {
		opt(sm)
	}
	...
	sm.Neuron = MakeNeuron(sm.Inputs, sm.Outputs, sm.BatchSize, sm.g, u, dt)
	return sm
}
